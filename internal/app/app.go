...
import (
	...
	"github.com/lacsar712/batcycle/internal/airflow"
	"github.com/lacsar712/batcycle/internal/clock"
	"github.com/lacsar712/batcycle/internal/config"
	"github.com/lacsar712/batcycle/internal/cellbus"
	"github.com/lacsar712/batcycle/internal/formloop"
	"github.com/lacsar712/batcycle/internal/fsm"
	"github.com/lacsar712/batcycle/internal/interlock"
	"github.com/lacsar712/batcycle/internal/model"
	"github.com/lacsar712/batcycle/internal/voltage"
	"github.com/lacsar712/batcycle/internal/store"
)
...
type App struct {
	...
	plant        *formloop.TowerPlant
	towerFSM     *fsm.TowerFSM
	zones        *formloop.ZoneTable
	...
	cellbus      *cellbus.Controller
	holdMgr      *voltage.HoldWindowManager
	holdEval     *voltage.HoldWindowEvaluator
	gradAudit    *voltage.OrderedGradientValidator
...
func New(cfg config.Config) (*App, error) {
	...
	zones, err := formloop.NewZoneTable(towerID, cfg.ZoneCount, plenumID, cfg)
	if err != nil {
		return nil, err
	}
	plant := formloop.NewTowerPlant(cfg, clk, mem)
	...
	for i, z := range zones.Zones() {
		...
		plant.RegisterSensor(voltage.NewSensor(sensorID, z.Zone))
		targets = append(targets, cfg.TargetMoistPct)
	}
	if err := plant.InitProfile(zoneIDs, targets); err != nil {
		return nil, err
	}
	ventBank, err := cellbus.NewVentBank(zoneIDs, "vent-damper")
	if err != nil {
		return nil, err
	}
	...
	holdMgr := voltage.NewHoldWindowManager()
	a := &App{
		...
		gradAudit:    voltage.NewOrderedGradientValidator(zoneIDs, cfg.MaxGradientDeltaPct, 0.5),
		guard:        interlock.NewGuard(guardPairs),
		batchCancels: make(map[model.TowerID]context.CancelFunc),
	}
	a.cellbus = initDehumidController(ventBank, cfg.MaxGradientDeltaPct, func(readings []model.MoistureReading) float64 {
