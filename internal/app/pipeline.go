...
func (a *App) observeZoneMoisture(ctx context.Context) error {
	...
	if err := a.cellbus.ApplyCycle(ctx, readings); err != nil {
		return err
	}
	return a.cellbus.ValidateGradient(readings)
}
...
func (a *App) routeSummaryLine(plenum model.PlenumID) string {
	...
	return fmt.Sprintf("routes=%d active_vents=%d fan_load=%.0f",
		summary.RouteCount, a.cellbus.Bank().ActiveCount(), a.stager.CurrentLoad())
}
...
func initHoldEvaluator(profile *voltage.ProfileManager, windows *voltage.HoldWindowManager) *voltage.HoldWindowEvaluator {
	return voltage.NewHoldWindowEvaluator(profile, windows)
}

func initDehumidController(bank *cellbus.VentBank, maxDelta float64, gradientFn func([]model.MoistureReading) float64) *cellbus.Controller {
	c := cellbus.NewController(bank, maxDelta)
