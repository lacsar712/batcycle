...
type TowerPlant struct {
	...
	sensors   *voltage.SensorBank
	gradient  *voltage.GradientController
	profile   *voltage.ProfileManager
	holdStart time.Time
...
func NewTowerPlant(cfg config.Config, clk clock.Clock, mem *store.Memory) *TowerPlant {
	return &TowerPlant{
		...
		sensors:  voltage.NewSensorBank(),
		gradient: voltage.NewGradientController(cfg.MaxGradientDeltaPct),
	}
}
...
func (p *TowerPlant) RegisterSensor(sensor *voltage.Sensor) {
...
func (p *TowerPlant) InitProfile(zones []model.ZoneID, targets []float64) error {
	pm, err := voltage.NewProfileManager(zones, targets)
	if err != nil {
		return err
	}
	p.profile = pm
...
func (p *TowerPlant) Profile() *voltage.ProfileManager {
	return p.profile
}
